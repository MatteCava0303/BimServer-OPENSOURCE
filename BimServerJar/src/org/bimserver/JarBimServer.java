...
public class JarBimServer {
	...
	public static void main(String[] args) {
		...
		for (String arg : args) {
			...
			}else if (arg.startsWith("incomingdir=")) {
				incomingdir = arg.substring(12);
				if (incomingdir.startsWith("\"") && incomingdir.endsWith("\"")) {
					incomingdir = incomingdir.substring(1, incomingdir.length() - 1);
				}
			}
		}
		final JarBimServer server = new JarBimServer();
		server.start(address, Integer.parseInt(port), homedir, incomingdir, "www");
	}
	...
	public void start(String address, int port, String homedir, String incomingdir,String resourceBase) {
		// Strange hack needed for OSX
		if (homedir.startsWith("\"") && homedir.endsWith("\"")) {
			homedir = homedir.substring(1, homedir.length()-2);
		}
		if (incomingdir.startsWith("\"") && incomingdir.endsWith("\"")) {
			incomingdir = incomingdir.substring(1, incomingdir.length()-2);
		}
		...
		bimServerConfig.setIncomingDir(Paths.get(incomingdir));
